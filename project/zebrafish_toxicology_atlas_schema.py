...
# Generation date: 2026-02-26T17:13:27
...
class Experiment(ZappEntity):
    """
    Group of observations (phenotypic outcomes and their control) that are linked by a common experiment and subject
    that are part of a study.
    """
    _inherited_slots: ClassVar[list[str]] = []
...
class Control(ZappEntity):
    """
    Information about controls used in the experiment, including the type of control (wildtype vs mutant, treated vs
    untreated) and vehicle information if applicable.
    """
    _inherited_slots: ClassVar[list[str]] = []
...
class ExposureEvent(ZappEntity):
    ...
    route: Optional[Union[str, "ExposureRouteEnum"]] = None
...
slots.route = Slot(uri=ZEBRAFISH_TOXICOLOGY_ATLAS_SCHEMA.route, name="route", curie=ZEBRAFISH_TOXICOLOGY_ATLAS_SCHEMA.curie('route'),
                   model_uri=ZEBRAFISH_TOXICOLOGY_ATLAS_SCHEMA.route, domain=None, range=Optional[Union[str, "ExposureRouteEnum"]])
